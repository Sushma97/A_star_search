...
class WordLadderState(AbstractState):
    ...
    def __lt__(self, other):    
        ...
        state = self.dist_from_start + self.h
        other_val = other.dist_from_start + other.h
        if state < other_val:
            return True
        elif state == other_val:
            if self.tiebreak_idx < other.tiebreak_idx:
                return True
        return False
...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        state = self.flatten_list(self.state)
        index = state.index(0)
        ...
        def up_neighbour(state, index):
            position = list(state)
            position[index], position[index - 3] = position[index - 3], position[index]
            return tuple(position)

        def down_neighbour(state, index):
            position = list(state)
            position[index], position[index + 3] = position[index + 3], position[index]
            return tuple(position)

        def left_neighbour(state, index):
            position = list(state)
            position[index], position[index - 1] = position[index - 1], position[index]
            return tuple(position)

        def right_neighbour(state, index):
            position = list(state)
            position[index], position[index + 1] = position[index + 1], position[index]
            return tuple(position)
        ...
        if not bottom(index):
            new_state = EightPuzzleState(self.change_to_array(down_neighbour(state, index)), self.goal,  self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
            nbr_states.append(new_state)
        if not left(index):
            new_state = EightPuzzleState(self.change_to_array(left_neighbour(state, index)), self.goal, self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
            nbr_states.append(new_state)
        if not top(index):
            new_state = EightPuzzleState(self.change_to_array(up_neighbour(state, index)), self.goal, self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
            nbr_states.append(new_state)
        if not right(index):
            new_state = EightPuzzleState(self.change_to_array(right_neighbour(state, index)), self.goal, self.dist_from_start + 1, self.use_heuristic, self.zero_loc)
            nbr_states.append(new_state)

        return nbr_states

    def change_to_array(self, list):
        return np.array(list).reshape(3, 3).tolist()
    ...
    def compute_heuristic(self):
        ...
        board = self.flatten_list(self.state)
        goal = self.flatten_list(self.goal)
        total = sum(abs(b % 3 - g % 3) + abs(b // 3 - g // 3) for b, g in ((board.index(i), goal.index(i)) for i in range(1, 9)))
        # total = manhattan(self.flatten_list(self.state), self.flatten_list(self.goal))
        return total
...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for neighbour in neighboring_grid_locs:
            new_state = SingleGoalGridState(neighbour, self.goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors)
            nbr_states.append(new_state)
        return nbr_states
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for neighbour in neighboring_locs:
            if neighbour in self.goal:
                goal = copy.deepcopy(self.goal)
                goal = list(goal)
                goal.remove(neighbour)
                goal = tuple(goal)
                new_state = GridState(neighbour, goal, self.dist_from_start + 1, self.use_heuristic,
                                      self.maze_neighbors, self.mst_cache)
            else:
                new_state = GridState(neighbour, self.goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
            nbr_states.append(new_state)
        return nbr_states
    ...
    def compute_heuristic(self):
        ...
        if len(self.goal) == 1:
            mst = 0
        elif self.mst_cache.get(self.goal, None):
        ...
        for goal in self.goal:
            mindist = min(mindist, manhattan(self.state, goal))
        total = mindist + mst
        return total
